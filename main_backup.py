...
def ocr_image(image: Any, language: str, enhance: bool) -> tuple[str, float]:
    import pytesseract
    ...
    text = clean_text(pytesseract.image_to_string(image, lang=language, config="--oem 3 --psm 3"))
    return text, confidence_from_data(data)


def extract_pdf(path: Path, language: str, enhance: bool) -> dict[str, Any]:
    import pymupdf
    ...
    for page in document:
        ...
        page_text, confidence = ocr_image(image, language, enhance)
    ...
    return {
        ...
        "engine": "Tesseract OCR · 300 DPI render",
        "warnings": ["Vérifiez les passages signalés si la confiance est inférieure à 85 %."] if confidences and sum(confidences) / len(confidences) < 0.85 else [],
    }


def extract_image(path: Path, language: str, enhance: bool) -> dict[str, Any]:
    from PIL import Image

    with Image.open(path) as image:
        text, confidence = ocr_image(image, language, enhance)
    return {
        "documentType": "image",
        "pages": 1,
        "text": text,
        "confidence": confidence,
        "engine": "Tesseract OCR · image prétraitée" if enhance else "Tesseract OCR",
        "warnings": ["Vérifiez les passages signalés si la confiance est inférieure à 85 %."] if confidence < 0.85 else [],
    }
...
def main() -> None:
    ...
    try:
        if request["mimeType"] == "application/pdf" or suffix == ".pdf":
            result = extract_pdf(temp_path, language, enhance)
        else:
            result = extract_image(temp_path, language, enhance)
        result["tables"] = []
